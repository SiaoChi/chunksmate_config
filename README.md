...
| `latest_version` | 目前最新版本。低於此版本但高於 min → 軟性提示，用戶可選擇「稍後再說」 |
...
### 軟性更新（Soft Update）
- 條件：`ios_min_version <= currentVersion < latest_version`
- Modal 顯示「稍後再說」按鈕，用戶可暫時關閉
- **注意**：目前每次開啟 App 都會重新提示，無「今日不再顯示」機制

### 不提示
- 條件：`currentVersion >= latest_version`
